...
def do_prune_tool_arguments(tool: CommandToolBuilder, valid_tinput_ids: set[str]) -> None:
    ...
    items_to_delete: list[int] = []
    inputs_args = ordered_tool_inputs_args(tool)
    
    for i, (idx, entity) in enumerate(inputs_args):
        if isinstance(entity, ToolArgument):
            should_delete = has_invalid_input_ref(entity, tool, valid_tinput_ids)
            if should_delete:
...
def has_invalid_input_ref(targ: ToolArgument, tool: CommandToolBuilder, valid_tinput_ids: set[str]) -> bool:
...
def ordered_tool_inputs_args(tool: CommandToolBuilder) -> list[Tuple[int, ToolInput | ToolArgument]]:
    """
